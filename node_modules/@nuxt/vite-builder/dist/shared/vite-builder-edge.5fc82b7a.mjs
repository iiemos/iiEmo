...
import { resolve, join, dirname, isAbsolute, relative } from 'pathe';
import { logger, resolveModule, requireModule, isIgnored } from '@nuxt/kit';
import replace from '@rollup/plugin-replace';
import { resolve as resolve$1, sanitizeFilePath } from 'mlly';
...
function createViteNodeMiddleware(ctx, invalidates = /* @__PURE__ */ new Set()) {
  ...
  app.use("/module", defineLazyEventHandler(() => {
    ...
    node.shouldExternalize = async (id) => {
      ...
      if (result?.external) {
        return resolve$1(result.id, { url: ctx.nuxt.options.rootDir });
      }
      return false;
    };
    return async (event) => {
...
async function buildServer(ctx) {
  const useAsyncEntry = ctx.nuxt.options.experimental.asyncEntry || ctx.nuxt.options.vite.devBundler === "vite-node" && ctx.nuxt.options.dev;
  ctx.entry = resolve(ctx.nuxt.options.appDir, useAsyncEntry ? "entry.async" : "entry");
